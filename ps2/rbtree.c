 ...
 * In each cycle, I am checking whether the node is NULL and whether it is * a red node.
 * These conditions can be checked in O(1).
 */
...
bool is_red_black_tree(struct node * root) {
    if (isNodeRed(root))
        return false;
    int black_path = -1; // set as an default value to stand for the black path needed
